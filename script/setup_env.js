...
function DrawScene()
{
    var mvp = GetModelViewProjection( perspectiveMatrix, -2, 0, transZ, rotX, autorot+rotY );
    var mvp_sec = GetModelViewProjection( perspectiveMatrix, 2, 0, transZ, rotX, autorot+rotY );
    var m = GetModelViewProjection( perspectiveMatrix, 0, 0, transZ, rotX, autorot+rotY );
...
function LoadObj( param )
{
    if ( param.files && param.files[0] ) {
        ...
        reader.onload = function(e) {
            ...
            var imageUrl = '../assets/2k_saturn.jpg';  
...
function LoadTexture(imageUrl) {
    var img = new Image();
    img.onload = function() {
        // Assuming i want to apply texture to BoxDrawer
        meshDrawer.setTexture(img);  // Or meshDrawer.setTexture(img) for MeshDrawer
        DrawScene();  // Redraw the scene after texture is loaded
    };
    img.src = imageUrl;
}
...
function toggleQuantumPanel() {
    var controlPanel = document.getElementById('InformationPanel');
    controlPanel.classList.toggle('open');
}
